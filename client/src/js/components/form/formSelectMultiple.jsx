...
import { UpperCaseEveryLetter } from "../../utils/formatString";
...
function FormSelectMultiple({
  ...
  useEffect(() => {
    const newValueArr = options.filter(
      (item, itemIndex) => selected[itemIndex]
    );
  ...
  const handleOnSelect = async (index) => {
    ...
    const newSelected = selected.map((item, itemIndex) =>
  ...
  return (
    <div className="form__selectmultiple form__select">
      <div className="select__box" onClick={() => setActive(!active)}>
        ...
        <img
          src={process.env.PUBLIC_URL + icons.dropdownTriangles}
          alt="icon"
        />
      </div>
      {active && (
        ...
        >
          {options.map((option, index) => (
            ...
            >
              <p>{UpperCaseEveryLetter(option[fieldName])}</p>
            </li>
          ))}
        </ul>
      )}
