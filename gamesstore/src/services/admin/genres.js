...
export async function addNewGenre(token, genre) {
    const { data } = await http.post(`${endPoint}/new`, genre, configHeader(token)).catch(ex => ex.response);
    return data;
}

export async function updateGenre(token, genreId, genre) {
    const { data } = await http.put(`${endPoint}/${genreId}`, genre, configHeader(token)).catch(ex => ex.response);
    return data;
}
