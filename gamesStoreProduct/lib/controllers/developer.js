...
exports.update_game = [
    ...
    async (req, res) => {
        ...
        const { error: isGameError } = gamesValidator.validateGameUpdate(
            _.pick(req.body, ["name", "tagId", "description", "price", "available"])
        );
    ...
    async (req, res) => {
        ...
        uploadImageGame(req, res, async (error) => {
            ...
            const game = gameSchema(
                _.pick(req.body, ["name", "price", "tagId", "description", "available", "stock"])
            );
            const uniqueTags = req.body.tagId.filter((value, index, array) => array.indexOf(value) === index);
