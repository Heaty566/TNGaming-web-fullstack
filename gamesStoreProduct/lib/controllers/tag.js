...
exports.get_all_tags = [
    ...
    async (req, res) => {
        ...
        const tags = await db.tags.find().toArray();
...
exports.add_new_tags = [
    ...
    async (req, res) => {
        ...
        if (isUnique) return res.status(400).json({ success: false, msg: "Tag is existed" });
...
exports.update_tags = [
    ...
    async (req, res) => {
        ...
        const { error: isTag } = tagsValidator.validateTag(_.pick(req.body, ["name"]));
        if (isTag) return res.status(400).json({ success: false, msg: isTag.details[0].message });
        ...
        await db.tags
            ...
            .catch(() => {
                ...
                res.status(400).json({ success: false, msg: "Error inserting tag document" });
            });
    },
];
