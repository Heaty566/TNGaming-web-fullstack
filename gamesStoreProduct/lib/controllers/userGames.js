...
exports.add_game_to_compartment = [
    isUser,
    async (req, res) => {
        //{gameId: _id}
        ...
        const isCompartment = Object.keys(user).includes(
            req.params.compartment
        );
        if (!isCompartment)
            return res
                .status(400)
                .json({ success: false, msg: `Compartment is invalid` });
        ...
        const isNew = user[req.params.compartment]
            .map((item) => item.toString())
            .indexOf(req.params.gameId);
        ...
        if (!updateUser) {
            ...
            return res
                ...
                .json({
                    status: false,
                    msg: `Update ${req.params.compartment} failed`,
                });
        }
...
exports.remove_game_from_compartment = [
    isUser,
    async (req, res) => {
        //{gameId: _id}
        const db = req.app.get("db");
        ...
        if (isNew !== -1)
            user[req.params.compartment] = user[req.params.compartment].filter(
                (item, index) => index !== isNew
            );
            ...
            user.cart.map(async (gameId) => {
                ...
                if (!game)
                    return Promise.reject({
                        success: false,
                        msg: "The game in your cart doesn't exist",
                    });

                const isUnique = user.library
                    .map((id) => id.toString())
                    .includes(game._id.toString());
                if (isUnique)
                    return Promise.reject({
                        success: false,
                        msg: `you owned ${game.name}`,
                    });
            ...
            .then(async (games) => {
                ...
                if (user.balance < totalPrice)
                    return res
                        ...
                        .json({
                            success: false,
                            msg: "Your balance doesn't enough money",
                        });

                //create order schema
                // inserting order
                const order = await db.orders.insertOne(
                    orderSchema({ userId: user._id, cart: games, totalPrice })
                );
                if (!order) {
                    logger.error("Error Inseting game Order");
                    res.status(400).json({
                        success: false,
                        msg: "Update your order failed",
                    });
                }
                ...
                user.library = user.library.concat(
                ...
                if (!updateUser) {
                    ...
                    return res
                        ...
                        .json({
                            success: false,
                            msg: "Update your account failed",
                        });
                }

                return Promise.all(
                    games.map(async (game) => {
                        const developer = await db.users.findOne({
                            _id: ObjectId(game.developerId),
                        });
                        if (!developer) {
                            logger.error(
                                "Update find developer on order failed"
                            );
                            return Promise.reject({
                                success: false,
                                msg: "Update your account failed",
