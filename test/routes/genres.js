...
router.get("/:id", async (req, res) => {
    ...
    //checking Id
    const { error: isId } = isObjectId("Genre", req.params.id);
    if (isId) return res.status(400).json({ success: false, msg: isId.details[0].message });

    //gettting genre from database
    const genre = await db.genres.find({ _id: ObjectId(req.params.id) }).toArray();
...
router.put("/:id", async (req, res) => {
    ...
    if (!genre) return res.status(400).json({ success: false, msg: "Genre with the given Id was not found" });
